...
void main()
{
    ...
    float r1 = (rand(vec2(time,4.321)) -0.5)*6.;
    float r2 = (rand(vec2(time,1.234)) -0.5)*6.;

    if(gl_FragCoord.y > 1. && gl_FragCoord.y < resolution.y - 1. && gl_FragCoord.x > 1. && gl_FragCoord.x < resolution.x - 1.){
        c1 = box(p, vec2(r1,r2), vec2(1.));
        r1 = (rand(vec2(time,54.321)) -0.5)*8.;
        r2 = (rand(vec2(time,10.234)) -0.5)*4.;
        c2 = box(p, vec2(r1,r2), vec2(1.));
    }
    
    vec2 uv =  ( gl_FragCoord.xy / resolution.xy );

    vec4 hoge = texture2D(backbuffer, (uv * 1.2)-0.1);
